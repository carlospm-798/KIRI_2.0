...
void connect_to_esp32(SOCKET *sock, struct sockaddr_in *server) {
    ...
    if (valread > 0) {
        ...
        // Verificar si el mensaje tiene el carácter de inicio y fin.
        char startChar = '<';
        char endChar = '>';
        char* start = strchr(buffer, startChar);
        char* end = strrchr(buffer, endChar);

        if (start != nullptr && end != nullptr && end > start) {
            // Extraer el mensaje entre los caracteres de inicio y fin.
            *end = 0;  // Terminar el mensaje en el carácter de fin.
            printf("Response received: %s\n", start + 1);  // Imprimir el mensaje sin los delimitadores.
        } else {
